...
export class AnalyticsService {
  async getDashboardMetrics() {
    const lowStockQuery = await db.query(
      'SELECT * FROM products WHERE current_quantity <= min_quantity LIMIT 10'
    );

    const pendingOrdersQuery = await db.query(
      `SELECT * FROM orders WHERE status IN ('pending', 'preparing') LIMIT 10`
    );

    const staffQuery = await db.query(
      `SELECT * FROM staff WHERE DATE(hire_date) = CURRENT_DATE`
    );

    const costQuery = await db.query(
      `SELECT COALESCE(SUM(total_cost), 0) as daily_cost FROM orders WHERE DATE(created_at) = CURRENT_DATE`
    );

    const revenueQuery = await db.query(
      `SELECT COALESCE(SUM(total_price), 0) as daily_revenue FROM orders WHERE DATE(created_at) = CURRENT_DATE AND status = 'served'`
    );
